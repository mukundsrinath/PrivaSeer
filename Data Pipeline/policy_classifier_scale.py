...
This file scales the use the the random forest classifier to classify privacy policies.
...
en_hash_values = {}
with open(METADATA_DIRECTORY, encoding='utf-8') as f:
    metadata = f.readlines()
for line in metadata:
    line = json.loads(line)
    if line['language'] == 'en':
        en_hash_values[line['hash']] = line
...
batches_generated = os.listdir(DIRECTORY)
for batched_files in tqdm(batches_generated):
    count = 0
    with open(DIRECTORY+batched_files, encoding='utf-8') as f:
        data = f.readlines()
    processed_urls = []
    for line in data:
        line = json.loads(line)
