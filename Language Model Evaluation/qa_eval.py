...
#_train = []
#for i in range(len(train.question.values)):
...
class BERTClass(torch.nn.Module):
    def __init__(self):
        ...
        #self.l1 = RobertaModel.from_pretrained('/data/mus824/python-files/nessie/')
...
def train(model, optimizer, _train, train_labels, validation, validation_labels, num_epochs = 5, eval_every = 1158, file_path = destination_folder, best_valid_loss = float("Inf")):
    ...
    for epoch in range(num_epochs):
        for t_labels, text_ids, text_masks in batch(_train, train_labels):
            ...
            loss_func = nn.CrossEntropyLoss(weight=torch.Tensor([1,1]).double().to(device))
            ...
            if global_step % eval_every == 0:
                ...
                with torch.no_grad():  
                    for val_labels, val_text_ids, val_text_masks, in batch(validation, validation_labels):
                        ...
                        valid_loss_func = nn.CrossEntropyLoss(weight=torch.Tensor([1,1]).double().to(device))
